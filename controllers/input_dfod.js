...
export const deleteDFOD = async (req, res) => {
  ...
  try {
    ...
    await conn.query("UPDATE input_dfod SET status = 'deleted' WHERE id_input_dfod = ?", [id_input_dfod]);
      
    // 2. HAPUS entri dari laporan_keuangan (Bukan di-update jadi 0)
    const jenisTransaksi = mapJenisTransaksi(data.jenis_pembayaran);
    if (jenisTransaksi) {
      // Hapus entri Kas/Transfer
      await conn.query(
    ...
    await conn.query("DELETE FROM log_input_dashboard WHERE id_input_dfod = ?", [id_input_dfod]);
    
    await conn.commit();
    res.status(200).json({ message: "Soft delete berhasil, laporan keuangan dibersihkan" });
  } catch (err) {
    if (conn) await conn.rollback(); 
    res.status(500).json({ error: "Internal Server Error" });
  } finally {
...
export const restoreDFOD = async (req, res) => {
  ...
  try {
    ...
    await conn.query("UPDATE input_dfod SET status = 'active' WHERE id_input_dfod = ?", [id_input_dfod]);
      
    // 3. INSERT KEMBALI ke laporan_keuangan (Karena sebelumnya sudah di-delete)
    const jenisTransaksi = mapJenisTransaksi(data.jenis_pembayaran);
    if (jenisTransaksi) {
      await conn.query(
        "INSERT INTO laporan_keuangan (tanggal, jenis_transaksi, nominal) VALUES (?, ?, ?)",
    ...
    await conn.commit();
    res.status(200).json({ message: "Restore berhasil" });
  } catch (err) {
    if (conn) await conn.rollback();
    res.status(500).json({ error: "Internal Server Error" });
  } finally {
...
export const deletePermanentDFOD = async (req, res) => {
  ...
  try {
    ...
    const { id_input_dfod } = req.body;
    const [results] = await conn.query("SELECT * FROM input_dfod WHERE id_input_dfod = ?", [id_input_dfod]);
    const data = results?.[0];
    ...
    // Hapus dari laporan keuangan (Kriteria nominal sekarang akan cocok karena di fase soft delete sudah pakai DELETE)
    if (jenisTransaksi) {
      await conn.query(
        "DELETE FROM laporan_keuangan WHERE DATE(tanggal) = DATE(?) AND jenis_transaksi = ? AND nominal = ?",
        [data.tanggal_dfod, jenisTransaksi, data.nominal]
